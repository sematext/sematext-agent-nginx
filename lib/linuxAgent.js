    ...
    _formatLine: function (metric) {
      ...
      if (metric.sct === 'OS' && /collectd/.test(metric.name)) {
        ...
        if (metric.sct === 'OS') {
          ...
          if (metric.filters && metric.filter !== '') {
    ...
    start: function (agent) {
      ...
      if (cluster.isMaster) {
        var timerId = setInterval(function () {
          var time = Date.now()
          agent.addMetrics({ts: time, name: 'oslo', filters: '', value: [os.loadavg()], sct: 'OS'})
          if (this.isLinux) {
            ...
            linux.vmstat(function (err, vmstat) {
              ...
              linux.vmstatS(function (err, vmstats) {
                ...
                agent.addMetrics({
                  ...
                  name: 'osmem',
                  filters: '',
                  value: [vmstats.memory.used - (vmstats.memory.buffer + vmstat.cache) * 1024,
