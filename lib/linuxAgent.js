    ...
    _formatLine: function (metric) {
      ...
      if (metric.sct === 'OS' && /collectd/.test(metric.name)) {
        ...
        if (metric.sct === 'OS') {
          ...
          line = util.format('%d\t%s\t%d\t%s\t%s', now, metric.name, metric.ts, this.formatArray(metric.filters), this.formatArray(metric.value))
          logger.log(line)
        } else {
          line = util.format('%d\t%s\t%d\t%s\t%s', now, (metric.type || this.defaultType) + '-' + metric.name, metric.ts, this.formatArray(metric.filters), this.formatArray(metric.value))
          logger.log(line)
        }
      }
      return line
                ...
                data.forEach(function (netStat) {
                  if (netStat.bytes && netStat.Interface) {
                    agent.addMetrics({
                      ...
                      value: [Number(netStat.bytes.Receive), Number(netStat.bytes.Receive)],
                      sct: 'OS'
                    })
                  }
                })
