...
var dockerEnabled = process.env.DOCKER_AUTO_DISCOVERY || false
...
function NginxMonitor () {
  ...
  agentsToLoad.forEach(function (a) {
    try {
      var Monitor = a
      if (a === nginxAgent) {
        if (nginxUrl && nginxUrl !== '' && nginxUrl!=='empty')
  ...
  if (dockerEnabled) {
    ...
    var dockerDiscovery = new DockerDiscovery({
      ...
      startFunction: function (dockerEvent, containerInfo) {
        ...
        var port=0
        var useSSL = false
        var httpPort = process.env.NGINX_HTTPS_PORT||'80/tcp'
        ...
        if (process.env.SPM_DOCKER_NETWORK === 'host') {
          ...
          // connect via container networge e.g. bridge
          SpmAgent.Logger.info('using container network: '+ process.env.SPM_DOCKER_NETWORK )
          port = 80
          host = ontainerInfo.NetworkSettings.IPAddress
        } 
        if (useSSL) {
        ...
        if (port>0) {
          SpmAgent.Logger.info('start monitoring: '+ dockerNginxUrl + ' container: ' + dockerEvent.id)
          var na = new nginxAgent(dockerNginxUrl, filterValue)
